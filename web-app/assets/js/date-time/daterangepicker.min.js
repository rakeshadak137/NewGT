!function (b) {
    var a = function (k, s, i) {
        ...
        this.locale = {applyLabel: "Apply", clearLabel: "Clear", fromLabel: "From", toLabel: "To", weekLabel: "W", customRangeLabel: "Custom Range", daysOfWeek: moment()._lang._weekdaysMin, monthNames: moment()._lang._monthsShort, firstDay: 0};
        f = this.locale;
        ...
        if (q) {
            ...
            if (typeof s.startDate == "string") {
                this.startDate = moment(s.startDate, this.format).startOf("day")
            }
            if (typeof s.endDate == "string") {
                this.endDate = moment(s.endDate, this.format).endOf("day")
            }
            if (typeof s.minDate == "string") {
                this.minDate = moment(s.minDate, this.format).startOf("day")
            }
            if (typeof s.maxDate == "string") {
                this.maxDate = moment(s.maxDate, this.format).endOf("day")
            }
            if (typeof s.startDate == "object") {
                if (s.startDate instanceof Date) {
                    this.startDate = moment(s.startDate).startOf("day")
                } else {
                    this.startDate = s.startDate.startOf("day")
                }
            }
            if (typeof s.endDate == "object") {
                if (s.endDate instanceof Date) {
                    this.endDate = moment(s.endDate).endOf("day")
                } else {
                    this.endDate = s.endDate.endOf("day")
                }
            }
            if (typeof s.minDate == "object") {
                if (s.minDate instanceof Date) {
                    this.minDate = moment(s.minDate).startOf("day")
                } else {
                    this.minDate = s.minDate.startOf("day")
                }
            }
            if (typeof s.maxDate == "object") {
                if (s.maxDate instanceof Date) {
                    this.maxDate = moment(s.maxDate).endOf("day")
                } else {
                    this.maxDate = s.maxDate.endOf("day")
                }
            }
            if (typeof s.ranges == "object") {
                for (var m in s.ranges) {
                    var e = moment(s.ranges[m][0]).startOf("day");
                    var j = moment(s.ranges[m][1]).endOf("day");
            ...
            if (typeof s.dropdownAdjusts == "boolean") {
                this.dropdownAdjusts = s.dropdownAdjusts
            }
        }
        ...
        if (!q || (typeof s.startDate == "undefined" && typeof s.endDate == "undefined")) {
            if (b(this.element).is("input[type=text]")) {
                ...
                if (o.length == 2) {
                    this.startDate = moment(o[0], this.format).startOf("day");
                    this.endDate = moment(o[1], this.format).endOf("day")
                }
            }
        }
        ...
        this.container.find(".ranges").on("click", "button.clearBtn", b.proxy(this.clickClear, this));
        ...
        this.container.find(".calendar").on("change", "select.yearselect", b.proxy(this.updateYear, this));
        this.container.find(".calendar").on("change", "select.monthselect", b.proxy(this.updateMonth, this));
    ...
    a.prototype = {constructor: a, mousedown: function (c) {
        c.stopPropagation();
        if (!this.showDropdowns || b(c.target).not("select").length) {
            c.preventDefault()
        }
    }, updateView: function () {
        ...
        var e = moment(d[0], this.format).startOf("day");
        var c = moment(d[1], this.format).endOf("day");
        if (e == null || c == null) {
            return
        }
        if (c.isBefore(e)) {
        ...
        var d = (this.cleared ? null : this.startDate), c = (this.cleared ? null : this.endDate);
        this.cleared = false;
        this.cb(d, c)
    }, move: function () {
        ...
        if (d.hasClass("left")) {
            ...
            if (typeof this.dateLimit == "object") {
                var c = moment(f).add(this.dateLimit).endOf("day");
                if (i.isAfter(c)) {
                    i = c
                }
            }
        ...
        if (f.isSame(i) || f.isBefore(i)) {
            ...
            if (!f.isSame(this.startDate) || !i.isSame(this.endDate)) {
                this.changed = true
            }
            this.startDate = f;
            this.endDate = i
            ...
            if (f.isAfter(i)) {
                ...
                this.changed = true;
                this.startDate = f;
                this.endDate = moment(f).add("day", 1).endOf("day")
            }
        }
        this.leftCalendar.month.month(this.startDate.month()).year(this.startDate.year());
        this.rightCalendar.month.month(this.endDate.month()).year(this.endDate.year());
    ...
    }, buildCalendar: function (m, o, n) {
        var c = moment([o, m, 1]);
        var k = moment(c).subtract("month", 1).month();
        var j = moment(c).subtract("month", 1).year();
        var p = moment([j, k]).daysInMonth();
        var e = c.day();
        var h = [];
        for (var g = 0; g < 6; g++) {
            h[g] = []
        }
        var l = p - e + this.locale.firstDay + 1;
        if (l > p) {
            l -= 7
        }
        ...
        var f;
        if (n == "right") {
            f = moment([j, k, l]).endOf("day")
        } else {
            f = moment([j, k, l]).startOf("day")
        }
        for (var g = 0, d = 0, q = 0; g < 42; g++, d++, f = moment(f).add("day", 1)) {
            if (g > 0 && d % 7 == 0) {
                d = 0;
                q++
            }
        ...
        n += "</select>";
        return f + n
    }, renderCalendar: function (h, g, f, c) {
        var j = '<table class="table-condensed">';
        j += "<thead>";
        j += "<tr>";
        if (this.showWeekNumbers) {
            j += "<th></th>"
        }
        if (!f || f.isBefore(h[1][1])) {
            j += '<th class="prev available"><i class="icon-arrow-left"></i></th>'
        } else {
            j += "<th></th>"
        }
        var d = this.locale.monthNames[h[1][1].month()] + h[1][1].format(" YYYY");
        if (this.showDropdowns) {
            d = this.renderDropdowns(h[1][1], f, c)
        }
        j += '<th colspan="5" style="width: auto">' + d + "</th>";
        if (!c || c.isAfter(h[1][1])) {
            j += '<th class="next available"><i class="icon-arrow-right"></i></th>'
        } else {
            j += "<th></th>"
        }
        j += "</tr>";
        j += "<tr>";
        if (this.showWeekNumbers) {
            j += '<th class="week">' + this.locale.weekLabel + "</th>"
        }
        b.each(this.locale.daysOfWeek, function (n, m) {
            j += "<th>" + m + "</th>"
        });
        j += "</tr>";
        j += "</thead>";
        ...
        for (var l = 0; l < 6; l++) {
            j += "<tr>";
            if (this.showWeekNumbers) {
                j += '<td class="week">' + h[l][0].week() + "</td>"
            }
            for (var e = 0; e < 7; e++) {
                var i = "available ";
                i += (h[l][e].month() == h[1][1].month()) ? "" : "off";
                if ((f && h[l][e].isBefore(f)) || (c && h[l][e].isAfter(c))) {
                    i = " off disabled "
                } else {
                    if (h[l][e].isSame(g)) {
                        i += " active ";
                        if (h[l][e].isSame(this.startDate)) {
                            i += " start-date "
                        }
                        if (h[l][e].isSame(this.endDate)) {
                            i += " end-date "
                        }
                    } else {
                        if (h[l][e] >= this.startDate && h[l][e] <= this.endDate) {
                            i += " in-range ";
                            if (h[l][e].isSame(this.startDate)) {
                                i += " start-date "
                            }
                ...
                var k = "r" + l + "c" + e;
                j += '<td class="' + i.replace(/\s+/g, " ").replace(/^\s?(.*?)\s?$/, "$1") + '" data-title="' + k + '">' + h[l][e].date() + "</td>"
            }
            j += "</tr>"
        }
        j += "</tbody>";
