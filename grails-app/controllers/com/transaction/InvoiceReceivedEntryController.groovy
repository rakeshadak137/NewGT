...
class InvoiceReceivedEntryController {
    ...
    def dcReport(){
        ...
        def receiveData = InvoiceReceivedEntry.createCriteria().list {
            eq("branch",session['branch'])
            eq("isActive",true)
        ...
        receiveChildData = InvoiceReceivedEntryDetails.createCriteria().list {
            if(receiveData) {
        ...
        if(receiveChildData){
            receiveChildData.each {ch ->
                child.push([
                        fromCompany : (ch?.lrEntryDetails?.lrEntry?.fromCustomer?.accountName?:"") + " - " + (ch?.lrEntryDetails?.lrEntry?.toCustomer?.accountName?:""),
                        toCompany : ch?.lrEntryDetails?.lrEntry?.toCustomer?.accountName?:"",
                        invoiceNo : ch?.lrEntryDetails?.invoiceNo?:""
                ])
            }
        }
